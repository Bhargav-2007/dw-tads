"""Plane 4 runner (FastAPI + asynccontextmanager lifespan)."""
import os
import sys
...
app = FastAPI(title="DW-TADS Plane 4 — Fusion", lifespan=lifespan)


@app.get("/health")
async def health():
    return {"status": "ok", "plane": 4, "name": "plane4-fusion"}


@app.get("/ready")
async def ready():
    svcs = getattr(app.state, "services", [])
    return {"status": "ready", "services_running": len(svcs), "plane": 4}
...
async def metrics():
    ...
    for s in svcs:
        lines.append(
            f'dwtads_messages_total{service="{s.NAME}",plane="4"} {s.metrics_count}'
        )
    return "\n".join(lines)


if __name__ == "__main__":
    port = int(os.getenv("PLANE_PORT", "8004"))
