"""Plane 6 runner (FastAPI + asynccontextmanager lifespan)."""
import os
import sys
...
app = FastAPI(title="DW-TADS Plane 6 — Case", lifespan=lifespan)


@app.get("/health")
async def health():
    return {"status": "ok", "plane": 6, "name": "plane6-case"}


@app.get("/ready")
async def ready():
    svcs = getattr(app.state, "services", [])
    return {"status": "ready", "services_running": len(svcs), "plane": 6}
...
async def metrics():
    ...
    for s in svcs:
        lines.append(
            f'dwtads_messages_total{service="{s.NAME}",plane="6"} {s.metrics_count}'
        )
    return "\n".join(lines)


if __name__ == "__main__":
    port = int(os.getenv("PLANE_PORT", "8006"))
